...
public class Pack{
	...
	public void shufflePack(){
		dealPos = 0; 				// Reset Dealing
		if(numberSeed == 0L){ 	// Set the Randomizer and the Seed
		...
		for(int i=NumOfCards; i>0; i--){				// For all the cards in the Pack (starting at the top)
			int theIndex = randomizer.nextInt(i);	// Pick one at random. Use the number of Cards left as upper limit
		  try{
			shuffledList.addItem(allCards.removeItemAt(theIndex)); // remove it and put it in the New List
		  }catch(Exception e){
			System.out.println("Problem with shuffling!\n" + e); // There should be no Exceptions, for debugging only.
		  }
		} // End of for loop
		allCards = shuffledList; // Now make the Pack to be the shuffled List.
	}
	...
	public Card dealCard()throws DealException{
		if(dealPos > allCards.getLength()){
			throw new DealException("You have dealt the last card already!");
		}
	  try{
		...
		return dealtCard;
	  	...
	  	throw new DealException("There is no card there!\n" + e);
	  }
	}
